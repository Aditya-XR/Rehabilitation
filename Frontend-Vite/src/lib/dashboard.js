export const slotStatusClasses = {
...
export const contentTypeClasses = {
...
export function isSlotEditable(status) {
  return status === "available" || status === "cancelled"
}

export function getLatestReviewNote(booking) {
...
export function getPrimaryContentImage(content) {
  return content.images[0]?.url ?? ""
}

export function groupPublishedContent(items) {
  const order = [
    "hero",
    "section",
    "facility",
...
export function hasContactInfo(contactInfo) {
  if (!contactInfo) {
    return false
  }
